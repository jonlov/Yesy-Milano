...
angular.module('bootstrapLightbox').provider('Lightbox', function() {
    ...
    this.calculateImageDimensionLimits = function(dimensions) {
        if (dimensions.windowWidth >= 768) {
            return {
                ...
                'maxHeight': dimensions.windowHeight - 220
            };
        } else {
            return {
                ...
                // 86px = 52px as above
                //        + 34px outer height of .lightbox-nav
                'maxHeight': dimensions.windowHeight - 186
            };
        }
    };
    ...
    this.$get = ['$document', '$injector', '$uibModal', '$timeout', 'ImageLoader',
        function($document, $injector, $uibModal, $timeout, ImageLoader) {
             ...
             * @memberOf bootstrapLightbox.Lightbox
             */
            Lightbox.openModal = function(newImages, newIndex, modalParams, thumb) {
                Lightbox.title = newImages.title || '';
                Lightbox.images = newImages.images;
                Lightbox.setImage(newIndex, thumb);
                ...
                Lightbox.modalInstance = $uibModal.open(angular.extend({
                    ...
                    'controller': ['$scope', function($scope) {
                        ...
                        $scope.pagination = {
                            totalItems: $scope.Lightbox.images.length,
                            currentPage: 1,
                            itemsPerPage: 6
                        };

                        $scope.prevImage = function() {
                            var index = $scope.Lightbox.index - 1 + $scope.Lightbox.images.length;
                            $scope.setPage(Math.ceil((index % $scope.Lightbox.images.length + 1) / $scope.pagination.itemsPerPage));

                            $scope.Lightbox.setImage(index % $scope.Lightbox.images.length);
                        };

                        $scope.nextImage = function() {
                            var index = $scope.Lightbox.index + 1;
                            $scope.setPage(Math.ceil((index % $scope.Lightbox.images.length + 1) / $scope.pagination.itemsPerPage));
             ...
             * @memberOf bootstrapLightbox.Lightbox
             */
            Lightbox.setImage = function(newIndex, thumb) {
