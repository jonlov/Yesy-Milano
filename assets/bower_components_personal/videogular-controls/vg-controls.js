        ...
        function(VG_STATES, VG_UTILS) {
            return {
                ...
                link: function(scope, elem, attr, API) {
                    ...
                    var touchStartX = 0;
                    var touchFinishX = elem[0].scrollWidth;
                    ...
                    scope.onScrubBarMouseDown = function onScrubBarMouseDown(event) {
                        ...
                        // API.seekTime(offsetX * API.mediaElement[0].duration / elem[0].scrollWidth);
                    ...
                    scope.onScrubBarMouseMove = function onScrubBarMouseMove(event) {
                        ...
                        //         offsetX = elem[0].scrollWidth - 0.1;
                    ...
                    if (VG_UTILS.isMobileDevice()) {
                        ...
                        // document.documentElement.addEventListener('mouseup', scope.onScrubBarMouseUp);
                        // document.documentElement.addEventListener("mousemove", scope.onScrubBarMouseMove);
                    }
...
angular.module("com.2fdevs.videogular.plugins.controls")
    .run(
        ["$templateCache", function($templateCache) {
            $templateCache.put("vg-templates/vg-volume-bar",
              ...
              <div class="volumeBackground" ng-click="onClickVolume($event)" ng-mousedown="onMouseDownVolume()">\
    ...
    .directive("vgVolumeBar", ["VG_UTILS", function(VG_UTILS) {
        return {
            ...
            link: function(scope, elem, attr, API) {
                scope.mouseHoverVolume = false;
                scope.isChangingVolume = false;
                ...
                scope.onMouseDownVolume = function onMouseDownVolume() {
                    event.preventDefault();
                    scope.isChangingVolume = true;
                };

                scope.onMouseUpVolume = function onMouseUpVolume() {
                    if (scope.isChangingVolume) {
                        scope.isChangingVolume = false;

                        if (!scope.mouseHoverVolume) {
                            scope.volumeVisibility = "hidden";
                            scope.$apply();
                        }
                    }
                ...
                scope.onMouseMoveVolume = function onMouseMoveVolume(event) {
                    if (scope.isChangingVolume) {
                        ...
                        var offsetY = $(window).height() - event.clientY - volumeHeight;

                        if (offsetY >= volumeHeight)
                            offsetY = volumeHeight;
                        else if (offsetY <= 0)
                            offsetY = 0;

                        var value = offsetY / volumeHeight;
                ...
                document.documentElement.addEventListener("mouseup", scope.onMouseUpVolume);
                document.documentElement.addEventListener("mousemove", scope.onMouseMoveVolume);
